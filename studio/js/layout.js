...
export class Layout {
  ...
  collapsibleBlocks() {
    document.querySelectorAll('#panel-left .block').forEach((b) => {
      ...
      head.classList.add('fold'); head.title = 'Plegar / desplegar';
      if (this.s.blocks[b.id]) b.classList.add('collapsed');
      head.addEventListener('click', () => { b.classList.toggle('collapsed'); this.s.blocks[b.id] = b.classList.contains('collapsed'); this.save(); });
    });
  }
